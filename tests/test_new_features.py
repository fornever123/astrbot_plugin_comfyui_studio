...
def test_plugin_version_is_1_0_1_everywhere():
    assert 'PLUGIN_VERSION = "1.0.1"' in (PLUGIN_DIR / "main.py").read_text(encoding="utf-8")
    assert "version: 1.0.1" in (PLUGIN_DIR / "metadata.yaml").read_text(encoding="utf-8")
    assert "版本 v1.0.1" in (PLUGIN_DIR / "index.html").read_text(encoding="utf-8")
    assert "版本-1.0.1-" in (PLUGIN_DIR / "README.md").read_text(encoding="utf-8")
